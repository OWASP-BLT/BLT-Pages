...
async function loadRecentBugsFromAPI(grid) {
  ...
  const openIssues = issues.filter((i) => !i.pull_request).slice(0, 3);

  // Fetch latest comment for each bug (one request each, sorted desc)
  const latestComments = await Promise.all(
    openIssues.map(async (issue) => {
      if (issue.comments === 0) return null;
      try {
        const cRes = await fetch(
          `https://api.github.com/repos/${BLT_CONFIG.REPO_OWNER}/${BLT_CONFIG.REPO_NAME}/issues/${issue.number}/comments?per_page=1&sort=created&direction=desc`,
  ...
  const bugs = openIssues.map((issue, idx) => {
    let domain = null;
    const urlMatch = issue.body?.match(/### URL\s*\n\n(https?:\/\/[^\s\n]+)/);
    if (urlMatch) {
    ...
    return {
      ...
      domain,
      latest_comment: latestComments[idx],
    };
  });
